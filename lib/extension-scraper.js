...
function condensePlugin(plugin) {
  ...
  return {
    name: plugin.name,
    description: plugin.description != null ? ent.encode(plugin.description) : '',
    author: (plugin.author && plugin.author.name != null) ? plugin.author.name : '',
    url: plugin.url,
    version: gruntVersion,
    modified: plugin.time.modified,
  ...
  async.waterfall([
    ...
    function(results, callback){
      ...
      if (err) {
        ...
        var pluginData = JSON.stringify({ "extension-data": pluginList });
